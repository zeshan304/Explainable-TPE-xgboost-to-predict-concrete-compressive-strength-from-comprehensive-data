...
df_missing=df3.drop(columns=['Cmix_merge'])
...
plt.tight_layout()
plt.show()
# plt.savefig(f'results/{df_type}Missing_heatmap.svg', format='svg')
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=7, stratify=X['Cmix_merge'])
X_train_wo_valid, X_val, y_train_wo_valid, y_val = train_test_split(X_train, y_train, test_size=0.1, random_state=7, stratify=X_train['Cmix_merge'])
...
plt.savefig(f'results/{df_type}_predvsact.png', format='png')
plt.show()
...
plt.tight_layout()
plt.savefig(f'results/shap/{df_type}_waterfalldf1.svg', format='svg', dpi=600, bbox_inches='tight')
plt.show()


plt.figure()
shap.plots.waterfall(shap_values[3], max_display=11, show=False)
plt.savefig(f'results/shap/{df_type}_waterfalldf2.svg', format='svg', dpi=600, bbox_inches='tight')
plt.tight_layout()
plt.show()


plt.figure()
shap.plots.waterfall(shap_values[9], max_display=11, show=False)
plt.savefig(f'results/shap/{df_type}_waterfalldfall.svg', format='svg', dpi=600, bbox_inches='tight')
plt.tight_layout()
plt.show()
